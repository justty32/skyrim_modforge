...
- **配置方式**：config 檔（見作者 doc / GitHub），放對位置即生效。無 esp、無 behavior 編輯。
...
## 對 ModForge — 待辦
- **新增 BDI config 生成器**（roadmap 候選）：輸入 `{project, variables:[{name,type,default}], events:[name]}` → 輸出 BDI config。需先核對 BDI 0.13 的 config 格式（GitHub: max-su-2019/BehaviorDataInjector）。
