...
| `darksouls-port/` | 消費者 | 🔵 規劃完成 | 本機 DS Remastered 地圖移植成 Skyrim worldspace（首目標：北方不死院 m18_01_00_00）。MSB→spec placements、FLVER→glTF→NIF（吃 model-converter 反向）、碰撞為頭號牆。待 P0 spike（單塊 map piece 端到端）。**移植資產僅本機、不發佈**。 |
