...
func configure(terrain: TerrainGrid) -> void:
	_terrain = terrain
	add_layer("Skyrim.esm:0x0008C5")   # one layer to start (placeholder LTEX)
...
# Share the active layer's alpha + tint into the terrain so the mesh builder can blend it.
