...
| `splat_tool.gd` | `SplatTool`：紋理 alpha 筆（多層，每層 LTEX ref + alpha 格；paint 吸 active 層、radius/strength/erase 帶 falloff）+ base 貼圖 + 推真實貼圖混合給地形（WYSIWYG，無貼圖時退回頂點色 overlay）|
