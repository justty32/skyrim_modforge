...
> **信心標註**：Campfire 機制來自 mod-survey 對 `Campfire.bsa` 的原始碼逆向（[campfire.md](../../../../analysis/mod-survey/findings/skills-survival/campfire.md)，高信心）；JContainers API 來自原始碼（[jcontainers.md](../../../../analysis/mod-survey/findings/frameworks-runtime/jcontainers.md)）；Frostfall 是活範例（[frostfall.md](../../../../analysis/mod-survey/findings/skills-survival/frostfall.md)）。**但「對 NPC 而非營火開樹」「per-NPC 狀態橋接」是本檔的新設計推論——標 ⚠️ 者為待驗 unknown，未經主力機 / 原始碼確認前不可當事實。**
...
2. **Phase 1：玩家版 in-world 樹（繞過 U1/U2）**——照 Frostfall 模式掛一棵玩家樹到營火（Campfire 原生、全域 GLOB、零橋接）。驗證 in-world 星樹本體可生成可運作；只觸及 U4（generator）不觸及 U1/U2。
   - **✅ 零依賴 standalone 版 IN-GAME CONFIRMED（2026-06-21）**：`examples/inworld_skill_tree_standalone_spec.json` + `examples/MFSkillNode.psc`（native-compiler 編綠）。**只依賴 Skyrim.esm**——放棄 Campfire 的營火 radial menu，改最直白的 in-world 樹：3 顆水晶節點（vanilla `WispCrystal01.nif`）擺在白漫酒館 Bannered Mare（vanilla cell 0x01605E override），各掛自寫 `MFSkillNode` 腳本（純 vanilla 型別），點擊→gate（前置節點+點數）→給自訂 Fortify ability（Health/Stamina/CarryWeight）。**實機結果**：水晶可見、3 個 ability 都成功給到、gating 正確擋住未解鎖節點。⇒ **「JSON spec → 可點 in-world 節點樹 + 自寫 OnActivate 腳本 + 給效果 + 解鎖閘」整條 pipeline 在零外部 master 下成立**。
     - **漂浮星星（已修，2026-06-21）**：第一版用 `WispCrystal01.nif` → dynamic-havok clutter 會掉地上、也不是星星。修法＝用 Campfire 自己的 `_camp_intperkstars01.nif`（fixed-collision→漂浮、可點、就是天賦星）但**當 loose mesh 打包**（`assets:`→`Meshes/campfire/`），**不經 Campfire.esm**。其貼圖全 vanilla（重用原版天賦星座貼圖 `interface\INTPerkStars01.dds` 等），只需出 .nif，零紫貼圖風險。⇒ **要 Campfire 外觀但不裝 Campfire＝抽它的 nif loose 打包即可**（貼圖多半 vanilla）。
     - **第一版踩坑**：原放自建內景 cell `coc MFSkillTreeRoom` → 實機完全沒反應（新內景 cell 的 coc 是脆弱/未驗路徑）；改放 vanilla cell override + `coc WhiterunBanneredMare`（內建 100% 傳送）才成。**啟示：測試擺位優先用 vanilla cell override，別賭新內景 cell 的 coc。**
     - **視覺三修也 IN-GAME CONFIRMED（2026-06-21）**：① 紫貼圖 → 把 nif 引用的 9 個 .dds 全 loose 打包（不賭 vanilla 解析）；② 點亮 → 腳本 `PlayAnimation("OwnedWild")`（學會時+OnLoad 持久，鏡像 Campfire campperknode.psc）；③ 連線 → line activator 擺星間，學會播 `Unlock`，`downLine` 綁 line 的 placement ref。三者實機皆成功（星亮、線亮、不再紫）。連線用靜態 `rot(90,0,180)`（Frostfall 值）在固定垂直 layout 下對齊 OK。
     - **runtime 動態定向**：能做（line 腳本 OnLoad 讀兩端節點座標 atan2→SetAngle、距離→SetScale，即 Campfire `_Camp_PlaceableObjectBase` 做法），這次靜態就夠沒做。**Phase 3 generator 任意 layout 應走自動定向 line 腳本**，非每條硬寫 rotation。
     - **總啟示**：in-world 樹**不需要 Campfire**——vanilla cell + 可點 ACTI + 薄 OnActivate 腳本（AddSpell + GLOB gate + PlayAnimation 亮起）+ loose 打包 Campfire 星/線 nif（貼圖全 vanilla）就完整成立，零外部 master。Campfire 版（`inworld_skill_tree_spec.json`）留作 radial-menu 設計範本，不交付。
...
4. **Phase 3：ModForge generator — ✅ 落地 + IN-GAME CONFIRMED（2026-06-21，804 測綠）**：`skillTrees:` 一等 spec section，**macro-expansion** 架構——`Generator.ExpandSkillTrees` 在 `Build()` pass-0 把高階樹展開成既有低階記錄（points/rank GLOB + node/line ACTI + placements + `scripts:` 的 MFSkillNode 掛載），重用所有既有 pass。線方向/scale 在 **build-time C# 算好**（不需 runtime 腳本）：node[i] 堆在 origin+i*spacing，line 在中點、rot(90,0,180)、scale=spacing/65。`Generator.SkillTrees.cs` / `Spec.SkillTree.cs` / `Generator.Validate.SkillTrees.cs`；MFSkillNode.psc 嵌入 CLI、`package` 出 MFSkillNode.pex；art kit 經 `assets` loose 打包。**dump 驗證生成輸出與已實機確認的手刻版結構完全一致**（masters 只 Skyrim.esm、gating 鏈正確、3 星+2 線就位）。example `examples/skill_tree_spec.json`，docs SPEC-world §in-world skill trees、schema、CODE_MAP.world 同步。**MVP = 垂直線性鏈**；分支/2D layout（需對角線方向校準，無法離線驗）列為後續。
