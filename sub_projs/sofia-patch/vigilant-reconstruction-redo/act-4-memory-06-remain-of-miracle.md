...
Open verification:
- **(unverified: alias fill targets)** `questdiag` + `find` do not print alias fill refs for MeQ06. Alias `Alias_Pepe` fill = [`06A230 zzzCHInquisitorPepeMemory3`](/home/lorkhan/repo/ModForge/sub_projs/sofia-patch/game-data/mods/Vigilant/npcs.tsv:1069) is **(inference)** from FormID proximity; confirm via QUST record byte-dump or CK inspection.
