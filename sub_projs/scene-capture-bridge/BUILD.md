...
- **Linux (Manjaro) compile-verify path**: clang-cl + lld-link + [xwin](https://github.com/Jake-Shadle/xwin)（`xwin --accept-license splat --output ~/.xwin-cache`）+ vcpkg（`VCPKG_ROOT`）。**只做編譯驗證，不出貨** DLL（出貨走 Windows CI）。
