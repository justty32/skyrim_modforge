...
**並行 subagent 批次（3 功能跨領域並行開發）：⚠️ 結構驗證通過、待實機（2026-06-06）**——361 測試綠（+21）。用 3 個 worktree-隔離 subagent 同時開發互不重疊檔案領域的功能；我先離線解碼 vanilla 真值再下精確施工單，避免盲猜。**坑：worktree 從 stale base（32fd8a2）分出**（同 [[feature-swarm-branches]]），cherry-pick 回 master 時 package/test 檔衝突——以「keep both」逐檔解（SitTarget+Activate/Eat 相鄰加法），PackageTests 用 `--ours` 重置再貼回 agent 的 Activate/Eat 測試。三功能：
