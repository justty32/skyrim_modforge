...
**並行 subagent 批次 #2（3 功能跨領域並行開發）：✅ 實機驗證通過（2026-06-06）**——384 測試綠（+23）。玩家確認：自訂 firebolt（PROJ/EXPL）施法見彈+爆、Borin 拔自訂劍、`set MF_ShowGate to 1` 停吵架，全 PASS。**首測修掉兩個 bug**：① 武器 builder 把 templated 武器 clone 的 BasicStats 清成 0（spec 沒寫 damage 時）→ 0 傷害武器被 NPC 評分排在拳頭下不拔；修成 spec 留空保留 template 原值（+WeaponTests）。② showcase 誤用 scene-level `conditions` 閘 autoStart scene——**autoStart 由 controller 強制 `Scene.Start()`，繞過 scene begin-conditions**；改用 `autoStart.gateGlobal`（鐵律已寫進 SPEC）。**NPC 裝備物品偷竊**＝原版限制：未裝備物品免 perk 偷、已裝備武器/穿戴衣物需 Misdirection/Perfect Touch perk（非 ModForge 問題）；showcase 給備用未裝備劍示範免-perk 偷。第二輪 worktree 並行；檔案領域全互斥（A 占 Spec.cs/Build.cs/Validate.cs；B 占 NPC 檔；C 占 scene 檔）。**坑同前：worktree 全從 stale base 分出**（[[feature-swarm-branches]]），cherry-pick 衝突逐檔 keep-both 解（test 檔同名 class 用 `--ours`+貼回 / 重寫合併；Build.Scene.cs 3 處、Spec.Dialogue.cs 2 處 keep-both）。三功能：
- **Projectile (PROJ) + Explosion (EXPL) builder**（新 record，法術視覺）：`Spec.MagicFx.cs`+`Generator.Build.Explosions.cs`/`Generator.Build.Projectiles.cs`（EXPL 先建，PROJ 按 editorId 引用）+`Generator.Validate.MagicFx.cs`+pass-2 `WireMagicFxRefs`。自訂法術飛行彈鏈 EXPL←PROJ←MGEF(projectile=PROJ)←SPEL(Aimed)。`examples/projectile-explosion.json`（完整可施法 firebolt，vanilla 美術 path 全解碼驗證）。`MagicFxTests.cs`。
- **NPC inventory**（`NpcSpec.Items`=`NpcItemSpec{item,count}`）：`WireNpcs` pass 2 forward-ref-safe 填 `ContainerEntry`/`ContainerItem`；武器自動裝備、死亡掉落。`examples/npc-inventory.json`。
- **Scene 條件**（`SceneSpec.Conditions` scene-level + `ScenePhaseSpec.StartConditions`/`CompletionConditions` per-phase）：pass 2 `WireScenes` 經共用 `BuildCondition` 接（sceneConditionWires + phaseMap 對齊）。無條件＝行為不變。`examples/scene-conditions.json`。
- **一包一次測** `examples/showcase-multi2.json` → `~/skyrim_mods/ModForgeShowcase2.zip`：spell tome 學自訂 firebolt（PROJ/EXPL）→ 施法見彈+爆；Borin 帶自訂劍（NPC inventory）吵架+brawl；scene 條件閘 GLOB `MF_ShowGate`（==0 才播，`set MF_ShowGate to 1` 停）。解包確認 NPC Items=2、firebolt 鏈 4 record、scene-level condition。schema 暫未補（已落後，待專門 sync）。

**並行 subagent 批次 #1（3 功能跨領域並行開發）：✅ 實機驗證通過（2026-06-06）**——361 測試綠（+21）。玩家確認：showcase 綠光 + Borin 坐下 + 第2句轉頭看玩家 + brawl 全部生效。用 3 個 worktree-隔離 subagent 同時開發互不重疊檔案領域的功能；我先離線解碼 vanilla 真值再下精確施工單，避免盲猜。**坑：worktree 從 stale base（32fd8a2）分出**（同 [[feature-swarm-branches]]），cherry-pick 回 master 時 package/test 檔衝突——以「keep both」逐檔解（SitTarget+Activate/Eat 相鄰加法），PackageTests 用 `--ours` 重置再貼回 agent 的 Activate/Eat 測試。三功能：
- **Activate + Eat PACK template**（package 領域）：`Activate`=0x019B2D（slot 0 SingleRef→活化物件〔lever/door〕走位+活化、slot 2 Number；解自 `dunHillgrundsUnlockExteriorDoorActivate`）；`Eat`=0x019714（location sandbox-variant，固定 food/chair 搜尋，模仿 ApplySleepData）。`examples/package-activate.json`/`package-eat.json`。
- **Light（LIGT）builder**（新 record 領域）：`Spec.Lights.cs`+`Generator.Build.Lights.cs`+`Generator.Validate.Lights.cs`；color/radius/fadeValue/flags(`Light.Flag`)/falloff/fov/value/weight。自訂光源用既有 placements 放置（base=light editorId）。`examples/lights.json`。`LightTests.cs`。
- **Scene per-phase headtrack/facing**（scene 領域）：`ScenePhaseSpec` 加 `HeadtrackActor`(-2 預設=另一 actor／-1=無人／>=0=該 alias)/`HeadtrackPlayer`(看玩家,設 HeadtrackPlayer flag)/`FaceTarget`(預設 true)。預設行為 byte-identical 不變。`examples/scene-headtrack.json`。
- **一包一次測** `examples/showcase-multi.json` → `~/skyrim_mods/ModForgeShowcase.zip`：自訂綠色 flicker Light + autoStart scene（Borin 坐下 beat + 第2句 headtrackPlayer 看玩家 + brawlOnEnd）。解包確認 phase2 action `Flags=FaceTarget, HeadtrackPlayer`、Light radius420 color(70,230,110)。
