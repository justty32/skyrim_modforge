...
- **Wine 工具吃 Windows path**：`xWMAEncode.exe` / `FaceFXWrapper.exe` 在 Wine 下要 `Z:\...` 路徑；C# shell-out 前用 `winepath -w`。直接傳 Unix `/tmp/...wav` 會讓 xWMAEncode 報「Must specify input and output filenames」並導致 voice pipeline 降級成 loose `.wav`。
