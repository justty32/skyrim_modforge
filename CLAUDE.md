...
2. ~~**`MagicEffectSpec` script/VMAD + DualValueModifier**~~ **✅ 完成（2026-06-13，offline）**：① **Script-archetype MGEF**（VIGILANT 260/550，Boss 法術編排）原來**已支援**——通用 `scripts[]` ScriptAttach 反射任何有 writable VMAD 的 record，MGEF 已在 `recordsByEd`，故 `archetype:"Script"` + `scripts[]{targetEditorId:<mgef>}` 直接掛 Papyrus（`.psc` extends ActiveMagicEffect）；補了測試 + SPEC 說明。② **DualValueModifier** 新增 `MagicEffectSpec.SecondActorValue`/`SecondActorValueWeight`（78 MGEF 用）。
3. ~~**FLST builder + `GetIsInList`/`GetInCurrentLocation` CTDA**~~ **✅ 完成（2026-06-13，offline）**：① **FLST builder**——`formLists[]`（`FormListSpec` editorId + items 任意 record ref）`BuildFormLists`/`WireFormLists`，順序保留。② **`GetInCurrentLoc`** CTDA（Mutagen 真名是 `GetInCurrentLoc` 不是 `GetInCurrentLocation`，VIGILANT audit 也寫 GetInCurrentLoc:3，先前 probe 錯名字才誤判缺）——form arg = Location。③ **`GetIsInList` 無對應 ConditionData**（Mutagen 0.49）——FLST 改走既有 `*OrList` param（GetItemCount/GetEquipped/GetIsVoiceType/GetInWorldspace 都收 FormList），不是獨立函式（已寫進 SPEC/CODE_MAP）。
