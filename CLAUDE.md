...
**並行 subagent 批次 #2（3 功能跨領域並行開發）：⚠️ 結構驗證通過、待實機（2026-06-06）**——382 測試綠（+21）。第二輪 worktree 並行；檔案領域全互斥（A 占 Spec.cs/Build.cs/Validate.cs；B 占 NPC 檔；C 占 scene 檔）。**坑同前：worktree 全從 stale base 分出**（[[feature-swarm-branches]]），cherry-pick 衝突逐檔 keep-both 解（test 檔同名 class 用 `--ours`+貼回 / 重寫合併；Build.Scene.cs 3 處、Spec.Dialogue.cs 2 處 keep-both）。三功能：
