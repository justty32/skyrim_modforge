...
**設計來源：** `docs/superpowers/specs/2026-06-04-story-manager-probe-design.md`
...
把結果寫進 `docs/IDEAS.md` 第 9 節 Story Manager 段（或新筆記檔），含：Kill Actor SMEN FormID、Quest.Event 用的事件碼、PASS/FAIL、撞到的引擎 quirks。PASS → 階段二（spec schema + Generator.Build.StoryManager.cs + validator）另起 brainstorm。
