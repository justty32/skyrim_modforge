...
做某些事 → 取得身份（聖騎士 / 商人 / 冒險者 / 龍裔…）→ 身份賦予技能與常駐加成 + 解鎖身份專屬互動 → 互動回頭強化身份。近似 D&D 職業。NPC 預設用「當前主身份」稱呼你；身份可疊加、主身份用優先序純資料解析。取得走「讀書宣誓 / faction 會員」式。

**設計已 brainstorm 完成**：`docs/superpowers/specs/2026-06-06-identity-system-design.md`（封存待 round 2）。**相依前置**：PlayIdle scene-action 先獨立落地（聖騎士宣誓的下跪/祈禱動畫靠它）。子專案切分：B 身份系統本體 → C 身份對應互動（交易 UI、護衛任務…）。
...
- **階段二 spec 管線：✅ 實機驗證通過（2026-06-04）**。意圖導向落地：`QuestSpec.storyEvent`(event+conditions) + `QuestSpec.aliases`(fill `fromEvent:<slot>`|`forced:<ref>`)；事件表 `StoryManagerEvents`（只 KillActor）；`Generator.Build.StoryManager.cs`（pass 2，自動生 SMBN→SMQN + 清 StartGameEnabled）；`Generator.Validate.StoryManager.cs`。探針 builder/smprobe 退役、`smtree` 保留。樣本 `examples/story-manager-kill.json`。278 測試綠。設計/計畫見 `docs/superpowers/archive/specs/2026-06-04-story-manager-spec-pipeline-design.md` + `docs/superpowers/archive/plans/2026-06-04-story-manager-spec-pipeline.md`。
