...
- **階段二+ 擴充：✅ 已建（2026-06-04，待實機）**——三條軌並行（worktree subagent）整合上 master，289 測試綠。
  ...
  - **(C) Script Event 入口 — 研究尖兵完成（doc-only，未實作）**：見 `docs/superpowers/specs/2026-06-04-script-event-entry-spike.md`。關鍵結論：① vanilla Script Event SMEN 根 `Skyrim.esm:0x01379A`(Type=ScriptEvent)、Quest.Event 碼 `SCPT`、alias 槽 R1/R2/L1，**keyword 綁在分支條件上**（`GetEventData Keyword GetIsID <KYWD> == 1`，Mutagen `GetEventDataConditionData` 原生支援，ESP 側今天就能建）；② **Papyrus-on-Linux 可行（已實證）**：官方 `PapyrusCompiler.exe` 是 .NET assembly，`mono` + CK 的 `Scripts.zip`(14301 .psc 全源) 成功編出合法 `.pex`，不需 Caprica/wine；③ **單一通用 dispatcher 腳本可服務所有生成的 mod**（keyword/ref 是 runtime 參數，編一次裝 bytes）。建議下一步實作，最小驗證 = 編 dispatcher.pex + 一個 KYWD + `Event=ScriptEvent` quest，遊戲內呼叫 `Fire(kw, ref)` 確認 SM 啟動。
