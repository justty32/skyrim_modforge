...
- **Story Manager（SM）事件驅動任務：** 在任務中加入 `storyEvent` + `aliases`，建置時自動在原版事件根下配線 SMBN→SMQN。支援的事件：`KillActor`、`ChangeLocation`、`CastMagic`、`AddItem`、`Assault`、`ScriptEvent`。別名填充：`fromEvent:<槽>`、`uniqueActor:<ref>`、`forced:<ref>`。**遊戲內確認（2026-06-04）**，涵蓋所有變體（含 ESL 插件）。見 SPEC-dialogue-quests.md → 「Story Manager 任務」。
- **Script Event（自訂 Story 觸發）：** `ScriptEvent` 任務宣告一個 `keyword`；Papyrus 呼叫端透過 `MFStoryEventDispatch.Fire(kw, ref1, ref2, loc)` 發送事件，引擎將其路由到符合的 SM 任務。派發器 `.pex` 已嵌入 CLI，並在有任何 ScriptEvent 任務時由 `package` 自動複製到 `Scripts/`——無需每個模組單獨編譯 Papyrus。**遊戲內確認（2026-06-04）。**
