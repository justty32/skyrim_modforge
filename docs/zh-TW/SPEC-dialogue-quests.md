...
### dialogue（對話）
一個 `dialogue` 條目是顯示在任務分支下的玩家話題，可選擇性限定於一個說話者 NPC（一個 `GetIsID` 條件）。`questEditorId` 必須指向此 spec 中的一個任務；`speakerNpcEditorId`（若設定）必須指向一個 npc。`prompt` 為玩家的台詞；`responses` 為 NPC 的口語回應。
...
- 每個任務對應一個 **DialogView（DLVW）**，將其分支綁定到任務（沒有它，任務的玩家對話永遠不會被供應）；
- 每個說話 NPC 對應一個 **Hello** info（`Misc`/`Hello`/`SNAM='HELO'`），使 NPC 完全*可對話* — 使用 `npc.greeting` 設定台詞。

**結果 fragment（選擇台詞時執行某些操作）。** 對話選擇只能透過 Papyrus fragment *執行動作*（拿走金幣、加入跟隨系統、設定階段）——JSON 只持有靜態資料，永遠不含控制流。設定 `resultScript`（fragment 的 Scriptname，必須 `Extends TopicInfo` 並定義 `Function Fragment_0(ObjectReference akSpeakerRef)`）、`resultScriptSource`（`.psc`，由 `package` 編譯）和 `resultProperties`（綁定其 `Auto` 屬性——與 `scripts[]` 條目的 properties 同樣結構：`int`/`float`/`bool`/`string`/`object`）。建置時附加 INFO 的 `OnBegin` fragment VMAD（在玩家選取該行時觸發；只在需要等待完整語音回應後才執行的效果上使用 `OnEnd`）。設定 `goodbye: true` 可在台詞後關閉選單（vanilla 招募／解雇台詞都這樣做）。付費跟隨者招募範例見 `examples/follower_paid_spec.json` + `MFHirePaidRecruit.psc`。
...
```
從這一個條目，建置時會發出**完整的 vanilla 鏈**（對照 `dunIronbindBeemJaMourningScene` 上的 `scenediag`）：
- 宿主任務上每個 actor 對應一個 **QuestAlias**，各自以 `UniqueActor` 綁定到指定 NPC（使別名填入該特定角色）；
- **Scene（SCEN）**：其 `SceneActors` 參照**別名索引**（非 NPC FormKey）；其 `Phases` 是有序的 beat；每個階段對應一個 **Dialog `SceneAction`** 連結（說話別名、階段）→ 該行的話題，以*另一個* actor 作為 headtrack 目標，讓他們互相面對；
  - **per-phase 凝視覆寫**（可選）：一個階段可設定 `headtrackActor`（一個要看著的 actor `aliasId`；`-1` = 不看任何人；預設 = 另一個 actor）、`headtrackPlayer: true`（轉身面向**玩家**——與非預設的 `headtrackActor` 互斥）以及 `faceTarget`（預設 true）。用於某個 NPC 轉身直接對你說話的 beat。三者全部省略 = 行為不變。
  - **conditions**（可選的 CTDA 閘門，共用 `ConditionSpec`，在 pass 2 配線）：scene-level `conditions`（整個場景只在全部通過時才**啟動**）以及 per-phase `startConditions`（階段只在全部通過時才播放）/ `completionConditions`（一旦全部通過階段就結束）。沒有條件的場景與之前 byte 完全相同。見 `examples/scene-conditions.json`。
    - **重要 — scene-level `conditions` 只閘門引擎啟動的場景**（`beginOnQuestStart`，或非強制的引擎啟動）。一個 **`autoStart`（在場偵測）場景由 controller 腳本（`Scene.Start()`）強制啟動，會繞過 scene begin-conditions** — 所以 scene-level `conditions` 對 autoStart 場景**毫無作用**。要閘門在場觸發的場景，改用 `autoStart.gateGlobal`（controller 在啟動前檢查的 GLOB）。Per-phase `startConditions`/`completionConditions` 無論哪種方式在播放期間都仍會被評估。
- 每個階段對應一個 **Scene 子類型 DialogTopic**（Category=Scene，SNAM=`SCEN`）+ **INFO**，攜帶口語 `lines` + `emotion`。

> **執行時需求（非記錄錯誤）：**（1）兩個 NPC 必須**放置在彼此附近** — 在**同一個 cell** 中為每個 NPC 加入一個 `placements[]` 條目（他們必須同處一地才能交談）。（2）與所有任務對話一樣，場景只在**遊戲載入**時載入——測試新遊戲，或在宿主任務啟動後 `save`+`load`（建置會自動寫入 `.seq` 條目）。（3）無語音台詞閃過；安裝 **Fuz Ro D-oh** 並啟用字幕。**狀態：僅限結構**——`build`/`validate`/`dump` 已對照 vanilla 場景結構驗證；**尚未在遊戲中確認。** 見 `examples/scene_spec.json` 與 `lifelike/cookbook-advanced.md`。
...
##### 重播策略 — 控制何時／多頻繁重新觸發
預設情況下，只要玩家同處一地且 `cooldownSeconds` 已過，在場閘門就會每次重新觸發（無盡迴圈）。將以下任一項加入 `autoStart` 以控制重播（全部都與冷卻時間以 **AND** 結合）：
```jsonc
"autoStart": {
...
- **`playHour` / `playHourTolerance`** — 一個一天中的時間窗（controller 讀取遊戲內小時）。與真實時間冷卻無關——用於「只在中午」、「只在夜晚」等。
- **`gateGlobal`** — 通用機制：場景只在全域變數 `== 0` 時播放，且 controller 在其後立即 `SetValue(1)`。它接著保持關閉，直到某個**其他**生成的內容 `SetValue(0)` 它（一個對話結果腳本、一個任務階段 fragment、一個別名腳本、另一個事件）。這是「播放一次**直到某物重新武裝它**」。在 [`globals`](SPEC-items.md#globals-glob--shared-flags--counters--constants) 建立該 GLOB；將它重設為 0 是另外撰寫的（Papyrus）。見 `examples/scene-replay-policy.json`。

> 變更 `MFSceneBanterController.psc` 需重新編譯其 `.pex`（原生 `~/tools/papyrus-compiler` 並以 `MODFORGE_PAPYRUS_HEADERS` 指向 source cache，或 Wine+CK）。

#### actions — 非對話演出 beat（NPC 劇情演出，IDEAS §1b）
場景可以不只是說話。加入一個 `actions[]` 清單，場景就變成一齣小演出——*走到一個定點 → 等待 → 說話*。每個 action 是一個 vanilla 非 Dialog 的 `SceneAction`（從 `dunTolvaldsCaveCrownScene` / `BardSongs*` 場景透過 `scnscan` 解碼），在一段**階段索引窗口**內執行。一個只被 action 參照的階段可以有**空的 `lines`**——一個純 *beat 階段*。
```jsonc
...
```
- **`stages[]`** — `index`（唯一，**遞增**），`logEntry`（日誌文字；省略則為靜默里程碑），`completeQuest` / `failQuest`（設定 QuestLogEntry 旗標，當此階段到達時關閉／失敗任務——最多一個），`conditions`（可選的 CTDA 日誌條目門檻，以共用的 **ConditionSpec** 建立：`function`（一個 `GetStage`/`GetIsID`/… 名稱）、`comparison`（`==`/`>=`/… 或 `EqualTo`/`GreaterThanOrEqualTo`/…，預設 `>=`）、`value`、`param`（ref → 函式的 form 參數，例如 `GetStage` 的任務））。
- **`stages[].startUpStage`** — 標記引擎在任務啟動的那一刻**自動 `SetStage` 過去的階段**（vanilla QSDT「Start Up Stage」旗標）。這就是 **Story-Manager 觸發**的任務在**無外部 `SetStage`** 之下顯示其開場日誌條目／顯示其第一個目標的方式——沒有它，SM 啟動的任務會靜默地停在 stage 0。每個任務最多一個。（`dialogue`/`startGameEnabled` 任務通常不需要；SM 任務需要。）**遊戲內確認 2026-06-05。**
...
| `Arrest` | 守衛逮捕某個角色 | `guard`、`criminal` |
| `IncreaseLevel` | 玩家升級 | *（無——透過 `storyEvent.conditions` 閘門，例如 `GetLevel`）* |
...
| `findMatching:closest` / `findMatching:any` | `QuestAlias.Flag.MatchingRefInLoadedArea`（`closest` 再加 `MatchingRefClosest`） | 以**已載入區域中既有的 reference**填充，符合此別名的 `conditions`——`closest` 挑最近的符合者，`any` 挑第一個。比對過濾器是一個 CTDA 清單（與 `ConditionSpec` 同結構）配線到 `QuestAlias.Conditions`（例如 `HasKeyword ActorTypeNPC` = 最近的 NPC；`GetIsID <base>` = 最近的某個 base）。**至少需要一個條件。** 這是從 vanilla `MQGreybeardCall` Bystander 別名解碼出來的已載入區域「Find Matching Reference」機制——**不是** `FindMatchingRefNearAlias`（那只找編輯器中 linked-ref 的子項）。別名是否填充取決於執行時是否真有符合的 ref 在已載入區域。 |
...
| `script` / `scriptSource` / `scriptProperties` | — | 為此別名附加一個 Papyrus **別名腳本**（一個 extends `ReferenceAlias` 的腳本，儲存在任務的 `QuestAdapter.Aliases` VMAD，綁定到別名 ID）。它隨著**填入別名的任何 ref** 一起移動——包括 `createObject` 生成或 `findMatching` 比對到的、任何 base 物件腳本都無法觸及的 ref。典型用途是 `Event OnActivate(ObjectReference akActionRef)`：活化該別名 ref 就執行它（例如呼叫 `MFStoryEventDispatch.Fire(...)` 串接一個 story 事件）。`script` = Scriptname，`scriptSource` = 給 `package` 編譯的 `.psc`，`scriptProperties` 綁定其 Auto 屬性（與對話 `resultProperties` 同結構）。你提供編譯好的 `.pex`。遊戲內確認（2026-06-05）；可複用 helper `examples/MFSE_AliasActivate.psc`。 |

**一般任務上的別名（無 `storyEvent`）：** 同樣的 `aliases[]` 區塊在一般的 **StartGameEnabled** 任務上也能用——`forced` / `uniqueActor` / `createObject` / `findMatching` 填充以及別名 `script` 全都適用（只有 `fromEvent` 無效，因為沒有事件可拉取 ref——驗證器會標記它）。這些別名在任務啟動時填充（StartGameEnabled 任務 = 遊戲載入）。這讓一個純粹持續運作的任務能將 NPC/ref 強制填入別名、在其處生成物件，並攜帶一個 `OnActivate` 別名腳本——完全沒有 Story Manager 事件。遊戲內確認（2026-06-05）；示範 `examples/quest-alias-standalone.json`（強制玩家 → 在玩家處 `createObject` 箱子 → 打開它 → 別名 `OnActivate` 推進 + 關閉任務）。
...
**將 `Fire()` 接到真實觸發器。** 任何 Papyrus 情境都能呼叫派發器。一個可複用的模式是 magic-effect 腳本：將它附加到一個自訂 MGEF、設定一個 keyword 屬性，施放該法術就會以施法者作為 `ref1` 發出 story 事件：
...
`package` 會自動將這類腳本與嵌入的派發器 source 並排編譯，使 `Fire()` 在沒有任何本機 Papyrus 設定的情況下解析。同樣的結構也適用於對話 fragment、別名腳本或活化物——任何你能執行一行 Papyrus 的地方。

同樣的一行 `Fire()` 呼叫可從任何入口點運作——一個小型可複用的觸發器庫：

| 入口點 | 腳本（`extends …`） | 事件 | 範例 |
|-------------|----------------------|-------|---------|
| Magic effect（法術） | `ActiveMagicEffect` | `OnEffectStart` | `story-manager-magictrigger.json`（遊戲內 ✓） |
| Magic effect（藥水） | `ActiveMagicEffect` | `OnEffectStart` | `story-manager-potiontrigger.json`（同腳本，喝下觸發；遊戲內 ✓） |
| Activator | `ObjectReference` | `OnActivate` | `story-manager-activatortrigger.json`（拉動拉桿；遊戲內 ✓） |
| 對話行 | `TopicInfo` | `Fragment_0` | `story-manager-dialoguetrigger.json`（NPC 給予任務；遊戲內 ✓） |

四者全於 2026-06-05 遊戲內驗證。活化物注意：`model` 必須是 load order 中實際存在的 NIF 路徑——錯誤路徑會生成一個無錯誤的隱形物件。

前三者將一個帶 `Keyword` 屬性的腳本附加到一個記錄（MGEF / ACTI），透過 spec 的 `scripts[]` 設定。對話觸發器將腳本配線為一行的 `resultScript` + `resultScriptSource` + 一個 `TheKW` `resultProperty`。`package` 會自動將它們全部與嵌入的派發器 source 並排編譯。

另見 `examples/story-manager-scriptevent.json` + `examples/MFSE_TestTrigger.psc`（OnInit 測試）。
