...
一個以玩家為目標的 Follow 套件搭配市民身份食譜，可讓生成的 NPC 跟隨玩家**並在快速旅行後持續存在**（引擎會對執行「跟隨玩家」套件的角色一同進行快速旅行）。*移動層*本身不需要管理用的任務——僱用/解僱對話與跟隨派系（參見 [hireable-follower gotcha](gotchas.md)）只是讓玩家能夠自行切換開關的必要條件。
