...
"sounds": [
  { "editorId": "MFBellChimeSD",
    "files": [ "Sound\\fx\\mymod\\bell_chime_01.wav" ],
...
dotnet run --project src/ModForge.Cli -- validate examples/custom_asset_spec.json
...
`validate` 檢查：model 是 `.nif`、沒有 `Meshes\` 前綴、是相對路徑；音效至少有一個 `.wav`/`.xwm` 檔案；音效/類別/輸出模型 ref 可解析。
