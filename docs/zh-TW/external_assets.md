...
my_assets/
  ...
  Meshes/actors/mymod/anims/idle.hkx        # .hkx 放在 Meshes\ 下
```
...
"sounds": [
  { "editorId": "MFBellChimeSD",
    "files": [ "Sound\\fx\\mymod\\bell_chime_01.wav" ],   // 一個或多個；Data 相對，位於 Sound\ 下
    "category": "",            // ref → SNCT；空 -> Skyrim.esm:0x0172A1 AudioCategorySFX
    "outputModel": "",         // ref → SOPM；空 -> Skyrim.esm:0x0B4058（原版 SFX 輸出）
    "priority": 128,
    "staticAttenuation": 5.0 } // dB 衰減
],
...
```bash
# 1) 將您的資源製作到以 Data/ 為結構的來源目錄（Meshes/Textures/Sound/…）
# 2) 撰寫規格：模型路徑 + 音效 +（可選）`assets` 目錄
dotnet run --project src/ModForge.Cli -- validate examples/custom_asset_spec.json   # 路徑形狀 + refs
dotnet run --project src/ModForge.Cli -- package  examples/custom_asset_spec.json OutModDir
#    （或：package … OutModDir --assets /path/to/my_assets   以覆寫規格的 `assets`）
dotnet run --project src/ModForge.Cli -- dump OutModDir/MFCustomAssets.esp           # 驗證接線
find OutModDir -type f                                                                # 驗證打包
```
...
確認自訂資源能實際渲染/播放需要 Proton/Skyrim 啟動 — 詳見 [for_agent.md → 限制](for_agent.md#限制--請如實說明不要過度宣稱)。
