...
ModForge 目前實作十個模板——**Sandbox**（`Skyrim.esm:0x01C254`）、**Sleep**
...
（`Skyrim.esm:0x023B73`）、**SitTarget**（`Skyrim.esm:0x0A9277`）、**Activate**
（`Skyrim.esm:0x019B2D`）及 **Eat**（`Skyrim.esm:0x019714`）。撰寫對應的子物件
（`sandbox` / `sleep` / `travel` / `useMagic` / `patrol` / `follow` / `escort` / `sitTarget` /
`activate` / `eat`），建置便會填入該模板的 Data 插槽。若要指向 ModForge 尚未處理的模板
