...
以及一個 `effects` 清單。支援四種 effect 類型（`ability`、`entryPoint`、`addActivateChoice`、
`setText`）：
...
- **`addActivateChoice` / `setText`**（互動式 perk，Immersive Interactions 風格）— 在符合 effect
  `conditions` 的物件上加一個 `[E] <buttonLabel>` 選項，及/或改變 activation 提示文字。
  `entryPoint` 預設為 `Activate`。一個 `addActivateChoice` 會在物件上執行一個選用的 `spell`
  **及/或**一段 Papyrus `fragmentBody`（`akTargetRef` = 被 activate 的物件、`akActor` = 執行者 —
  例如 `akActor.SetSitState(akTargetRef)`）；`replaceDefault: true` 會取代預設 activation
  而非新增一個選項。`setText` 只設定 `text`（新提示文字）。有 `fragmentBody` 會讓 ModForge
  產生 `<perk>_Frags.psc`（extends Perk）並附加 **PerkAdapter** VMAD ——但只在 `package` 階段、
  且 `.pex` 已編譯完成時才會附加（VMAD 指向不存在的 `.pex` 會在載入時報錯；沒有 fragment、
  只有 spell 的選項則不需要 script）。實作範例：`examples/interactive_perk_spec.json`。
  ⚠ **離線限制：** entry-point + PerkAdapter 的 Mutagen *結構*已用反射驗證過，但位元組欄位
  （PerkAdapter 的 `version`/`objectFormat`、`IndexedScriptFragment` 未知欄位）與 Activate
  fragment 的**簽章**需要主力機上對一個真正的 Immersive Interactions perk 做 xEdit 位元組比對 —
  見 `WAIT_USER.md`。
