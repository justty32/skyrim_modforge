...
- **`Script`-archetype MGEF**（boss 法術邏輯、自訂的施加時行為）執行 **Papyrus**：設定
  `archetype: "Script"` 並用一個頂層 **`scripts[]`** 項目附加 script，該項目的
  `targetEditorId` 是此 MGEF 的 `editorId`（通用的 VMAD 附加方式適用於任何 record — MGEF 無需特殊
  接線）。`.psc` extends `ActiveMagicEffect`；`package` 會像任何其他 script 附加一樣編譯
  `scripts[].source`。`scripts[]` 的形狀見 [SPEC-quests](SPEC-quests.md) § scripts。
