...
**anchor**：`cell`（必填）、`position`、`kind`（標籤）。編兩個 `.pex` 需 Papyrus 機器（`package`
會 ship；build 條件式嵌入）。範例：`examples/living_npcs_spec.json`。

**MVP 範圍。** 具名卡司 + 抽象模擬 + 就地實體化 + 傳唱。**Phase 2+**（未做）：玩家互動（搶任務 /
雇用 / parley 敵對）、真 missive 任務目標（卡 roadmap #7–9 LocationAlias fill）、alignment/敵對
分岔、LAL 出身 seed 關係、匿名「群像」層。設計：`sub_projs/living-adventurers/`（idea #23 +
design.md）。
