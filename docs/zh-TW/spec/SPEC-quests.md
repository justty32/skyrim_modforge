...
- `fromJson: { file, key }`（選用）——**在 runtime 從外部 [PapyrusUtil JsonUtil] 檔讀取值**，而不用字面值。
  寫入的值變成 `JsonUtil.Get{Int,Float,String}Value("<file>", "<key>", <int/float/str 字面值>)`，其中字面值
  作為 **missing default**（JSON 鍵不存在時回傳）。`file` 相對於 `data/skse/plugins/StorageUtilData/`
  （`"../"` 可往上跳）；`key` 是 JsonUtil 鍵或點分 `.path`。這是「玩家可改／工具寫入的設定檔 → runtime 狀態」
  的橋接（例如從隨包 JSON 讀一個難度旋鈕進玩家 KV，供 mod 其餘部分讀）。
