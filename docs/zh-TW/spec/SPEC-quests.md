...
同時也帶 `storyEvent`（例如 `CastMagic`——見 [Story Manager quests](#story-manager-quests--event-driven-start)）的任務上。一個
...
`Stop()` 以重新待命。於是「*施放任何法術 → 存入 Endurance XP，在 rank 2 時取得 perk*」就是一個帶 persist
（以 `"player"` 為 key、附 `GetIsID Player` 事件條件）的 CastMagic 任務——沒有 NPC、沒有對話。完整
範例：`examples/npc_skill_persist_spec.json`。
