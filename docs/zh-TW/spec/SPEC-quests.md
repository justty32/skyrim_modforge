...
- **`stages[].startUpStage`** — 標記引擎在**任務啟動瞬間自動執行 `SetStage` 到的**階段
  ...
  **沒有外部 `SetStage`** 的情況下顯示其開場日誌條目／顯示其第一個目標——若沒有它，
  SM 啟動的任務會無聲地停留在階段 0。每個任務最多一個。（`dialogue`/`startGameEnabled`
  任務通常不需要；SM 任務則需要。）**IN-GAME CONFIRMED 2026-06-05。**
...
**StartGameEnabled** 任務上也適用——`forced` / `uniqueActor` / `createObject` / `findMatching` 填入與
別名 `script` 全都適用（只有 `fromEvent` 無效，因為沒有事件可從中拉取 ref——
validator 會標記它）。這些別名在任務啟動時填入（對 StartGameEnabled 任務而言即遊戲載入）。
這讓一個普通的常駐任務得以強制把一個 NPC/ref 塞進別名、在其處生成一個物件，並攜帶
一個 `OnActivate` 別名腳本——完全不需要 Story Manager 事件。遊戲內確認（2026-06-05）；示範
`examples/quest-alias-standalone.json`（forced 玩家 → 在玩家處 `createObject` 一個箱子 → 開啟它 →
別名 `OnActivate` 推進並關閉任務）。
