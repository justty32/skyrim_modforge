...
`BuildOptions.SkyrimDataPath` 可覆寫主外掛（Skyrim.esm，用於 template 複製 / 原版 cell 覆寫）的讀取位置；為 null 時依序回退至 `MODFORGE_SKYRIM_DATA`，再到預設 Steam 路徑。`spec.Esl`（預設 true）驅動 `IsSmallMaster`（light master：≤2048 筆新記錄，FormID 0x800–0xFFF；超出時 `PluginIo.Write` 會拋出明確錯誤）。
