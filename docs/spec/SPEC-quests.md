...
| `findMatchingLocation:<locTypeKeyword>[@<parentLocationAlias>]` | `QuestAlias.Type = Location` + match CTDAs | **Radiant LocationAlias (#7).** Fills a **Location**-type alias by "Find Matching Location" — pick a location whose **LocType keyword** matches (`<locTypeKeyword>` = an in-spec KYWD editorId or `Plugin.esm:0xID`), optionally narrowed to a child location **within** `@<parentLocationAlias>` (another Location alias in this quest). Emits a `LocationHasKeyword == 1` condition (the LocType) plus, when narrowed, a `GetInCurrentLocAlias == 1` condition (`LocationAliasIndex` = parent), and sets `StoresText` so the `<Alias=Name>` token renders the picked location. **Byte-verified vs shipping Missives `_M_QuestWhiterunKillBandit` (2026-06-21)** — the engine ignores `LocationAliasReference.Keyword` on a Location alias, so this is conditions-based, NOT a `LocationAliasReference`. The Missives radiant variety core: a Hold location, then a Dungeon/Inn location within it. |
...
| `packages` | `[]` | AI packages that **override this alias's actor** while the alias is filled (the `ReferenceAlias` "Packages" tab / ALPS, written to `QuestAlias.PackageData`), highest priority first. Each entry = an in-spec PACK editorId or `Plugin.esm:0xID`. **This is what actually drives a radiant escort/travel performance** — a package record alone (even with a correct `PackageTargetAlias`/`LocationFallback`) never runs unless it's listed on the actor's alias here. Byte-shape matches vanilla `MS13` Camilla alias. Demo `examples/radiant_package_spec.json` (VIP alias → `MFTravelToSafehouse` → Thoring (Windpeak Inn bartender) walks to the runtime-picked Safehouse location). |
...
(`findInLocationAlias:Dungeon#<bossLCRT>`). Demo `examples/radiant_alias_spec.json`. ✅ **Byte-verified
vs shipping Missives (2026-06-21, `questdiag`):** #7 = conditions-based (`LocationHasKeyword` +
`GetInCurrentLocAlias`) matching the Missives `Dungeon` alias field-for-field; #8 = `LocationAliasReference
{AliasID, RefType}` matching the Missives `Target` alias exactly. Remaining gate = in-game alias-fill
(`sqv MFRadiantBounty` after start; `ModForgeRadiantAlias.zip`). See `WAIT_USER.md`.
