...
**Phase-2/C 進度**：✅ #1 Adventurer 預設自動授予、✅ #2 `activeWhen` 情境條件、✅ #4 controller 主身份+手動覆寫（皆 in-game 2026-06-07）。🔶 #5 身份對應互動 **#5a 商人交易 UI（`openBarter`）+ #5b 護衛/跟隨任務（`identity`-gated escort quest，follow PACK gated on GetStage、`rewardItem`/`evaluateSpeakerPackages` TIF）**——built + 結構驗證 2026-06-07，**in-game 待測**。**未做**：#3 聲望/行為追蹤、龍裔首吼追加、#5 聖騎士 smite 細調。
