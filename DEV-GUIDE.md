...
- **觸發 A：單體過大** —— 某檔案、次級工作流、或 code 大檔體量太大。**客觀門檻：文檔（`.md`）單檔超過 8192 bytes；程式碼（`src/`、`examples/`）單檔超過 300 行**（文檔用 bytes 較穩定、不受行長影響；code 沿用行數）。門檻是**觸發檢視的訊號**、非硬性上限：本質不可分的單體（一份完整 spec／plan、一篇連貫解碼筆記）可超標保留——其中**已完成的 spec/plan 一律移 `*/archive/` 凍結**（不留現役夾、不拆）；凍結的 `*/archive/` 與鏡像程式碼領域的 `common/code-map/` 不套此門檻。
