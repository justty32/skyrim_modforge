...
| [captured-npcs-consumption.md](captured-npcs-consumption.md)（擷取器 ② NPC 外貌：capturedNpcs[] → NPC_ 記錄＋placement） | 2026-07-11 | [specs/ingame-scene-export-design.md](../specs/ingame-scene-export-design.md)（共用） | **待審/待動工**——① capturedItems 已落地（`1bed5dd`）；本檔規劃 ②：Phase 1 寫 TESNPC 配方（離線可做），Phase 2 烘焙臉（後面里程碑）；有 4 個開放問題（分段接受度/unique/faceMorph 驗證/activeEffects）|
