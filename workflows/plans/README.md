...
| [captured-npcs-consumption.md](captured-npcs-consumption.md)（擷取器 ② NPC 外貌：capturedNpcs[] → NPC_ 記錄＋placement） | 2026-07-11（同日細化至動工級 T1–T6） | [specs/ingame-scene-export-design.md](../specs/ingame-scene-export-design.md)（共用） | **待動工**——① capturedItems 已落地（`1bed5dd`）；Phase 1＝TESNPC 配方（離線可做完＋測），Phase 2＝烘焙臉（後面里程碑）；faceMorph index↔Mutagen 具名欄映射已離線鎖定（Q3 解決）；剩 Q1（灰臉中間態接受度）/Q2（unique 只 MINT）/Q4（activeEffects 不消費）待使用者拍板 |
