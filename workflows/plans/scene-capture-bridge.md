...
- **P2 修改（✅ MVP 離線落地 2026-07-10，僅限自己擺的 dynamic ref）**：`src/Editor.{h,cpp}`——numpad 5 選準星目標進編輯模式、8/2 前後、4/6 左右、**1/3 高低（與細摳②的 1379 全旋轉不同——spec 沒給高度軸，家具擺放對 Z 的需求遠大於第二旋轉軸；每鍵一個常數，要改隨時改）**、7/9 yaw、+/− 縮放（actor 略過）、0 commit、`.` cancel（還原快照）。編輯模式吞掉所有 numpad 鍵；未映射鍵印 scancode（numpad DIK 常數未經 F 區錨定，一輪亂按即可自我診斷）。`SetPosition`+`SetAngle` 後補 `Update3DPosition(true)`（否則靜物視覺可能不動——未實機驗）。**authored ref 誠實拒絕**（log 指向 overrides[] 契約）——拍板後這裡就是明示登記的掛點。**未做**：泛光選中效果、屬性 GUI（`*/`+`5`，需 record-representable 映射）、持續按鍵重複。
