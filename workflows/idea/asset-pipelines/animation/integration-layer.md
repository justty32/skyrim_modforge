...
- **Pandora Behaviour Engine+** (**the 2026 baseline**) — the modern, **open-source, cross-platform .NET 8** behavior patcher that replaced Nemesis/FNIS: generates patched behavior `.hkx` from mod-supplied patch data, **reads Nemesis-format mods + most FNIS mods** (so it subsumes both), faster, actively maintained. **This is the engine these notes assume.** Linux/headless caveats in [§6.0](linux-workflow-modforge.md#60-pandora-behaviour-engine--the-2026-baseline-what-replaced-nemesisfnis).
