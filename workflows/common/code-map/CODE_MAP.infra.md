...
| `Commands/CheckDependenciesTests.cs` | `check-dependencies`：TES4 header direct masters 對明示 `plugins.txt`／implicit 清單比對；enabled/disabled/missing、BOM/comment/case、衝突重複、空/全 disabled、壞輸入，以及真 CLI process exit 0/1/2 且不改輸入 bytes |
...
| `CatalogTests.cs` | 不需 Skyrim.esm 的 synthetic plugin round-trip：multiple source、FTS name/EditorID + type/plugin filter、exact FormKey/source lookup、load-order winner JSON（含 `--placeable` 同款 record-type filter、保留無 direct model 的合法 base；實際 exporter bytes 通過正式 Draft 2020-12 schema）、model path、source hash/path provenance、atomic failure、rerun replace 不重複。|
...
| CLI | `CheckDependenciesCmd.cs` | `check-dependencies <plugin> --plugins <plugins.txt> [--implicit-plugins <implicit.txt>]`：argv/header 編排、結果格式化與 exit 0/1；壞參數或輸入由頂層轉 exit 2 |
...
| Deps | `InstalledDependencies.cs` | 以 Mutagen TES4 header reader 取得 direct masters；跨平台一致解析 MO2 星號式 `plugins.txt` 與明示 implicit plugin 清單，產 enabled/implicit/disabled/missing 結果。不查 instance、檔案存在、版本或遞迴依賴 |
...
→ **說明文件**：[SPEC-distribution.md](../../../docs/spec/SPEC-distribution.md) · 格式調查 findings：[spid](../../../../../analysis/mod-survey/findings/frameworks-tools/spid.md)、[mcm-helper-config-json](../../../../../analysis/mod-survey/findings/frameworks-tools/mcm-helper-config-json.md)、[formlist-manipulator-config-core](../../../../../analysis/mod-survey/findings/frameworks-tools/formlist-manipulator-config-core.md)、[keyword-item-distributor-config-1](../../../../../analysis/mod-survey/findings/frameworks-tools/keyword-item-distributor-config-1.md)、[base-object-swapper-config](../../../../../analysis/mod-survey/findings/frameworks-tools/base-object-swapper-config.md)、[animobject-swapper-overview-config](../../../../../analysis/mod-survey/findings/frameworks-tools/animobject-swapper-overview-config.md)、[skypatcher-records-and-config](../../../../../analysis/mod-survey/findings/frameworks-tools/skypatcher-records-and-config.md)（D 組 ini pipeline，見 [roadmap/all-findings-gaps.md](../../roadmap/all-findings-gaps.md)）
...
**MCM Helper**（Idea D-2，ini path **in-game confirmed 2026-06-20**）：ModSetting 值進 `MCM/Settings/<plugin-stem>.ini`；`global` bool toggle 另生 per-menu setter + VMAD GLOB property（offline 2026-08-11，runtime 待確認）。光丟 config 不出選單，必須有 registration QUST + player alias；資料夾名＝plugin stem。examples：`mcm_config_spec.json`、`mcm_global_perk.json`。
