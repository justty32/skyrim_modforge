...
| Catalog | `Catalog.cs` / `Catalog.Schema.cs` / `Catalog.Export.cs` | 離線 SQLite/FTS5 catalog：generic major record（FormKey/plugin/type/EditorID/name/model path）+ source SHA-256/path/load-order provenance；DB schema/version gate 對舊 cache 明示 rebuild；保留 override occurrences，`export-json` 依明示 low→high order 只吐 winner、拒絕覆寫輸入 DB，flushed temporary→atomic replace，契約為 `schemas/scene-catalog.schema.json`。未做 record-specific schema。|
| CLI | `CatalogCmd.cs` | `catalog build/query/get/sources/export-json` facade；query/get/sources 可輸出人讀 TSV 或 camelCase JSON，export-json 供 scene-capture browser。|
