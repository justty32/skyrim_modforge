...
| Build P2 | `Generator.Build.Scripts.cs` `AttachDialogueResultScripts` | INFO OnEnd result fragment：user `ResultScript` 優先，否則 `needsAutoTif`（setStage / setPrimaryIdentity / **openBarter / rewardItem / evaluateSpeakerPackages / persist / syncPerks**，.pex 在才掛）→ 生成 TIF；綁 `OwningQuest`(setStage) + `MF_IdentityOverride` GLOB(setPrimaryIdentity) + `RewardItem` form(rewardItem) + **`PF_<i>` form(persist 的 form-valued entry) + `SyncPerk_<i>` perk(syncPerks node)**|
| Gen | `Generator.QuestFragments.cs` `GenerateDialogueFragmentSource(d, overrideCode)` | **多動作 TIF**：setStage→`OwningQuest.SetStage(N)`；overrideCode≥0→`MF_IdentityOverride.SetValue(code)`；**rewardItem→`Game.GetPlayer().AddItem(RewardItem, count)`；openBarter→`(akSpeakerRef as Actor).ShowBarterMenu()`；evaluateSpeakerPackages→`.EvaluatePackage()`**（後二共用 `__spk` cast）；**persist/syncPerks→`JContainersFragmentBody(d)`（見下）**。`DialogueFragmentScriptName` 任一 result 動作（含 persist/syncPerks）→ `TIF_<ed>`|
| Spec | `Spec.Persist.cs` | **Idea #20 Phase 0**：`PersistSpec`(storage/key/set[]) + `PersistEntrySpec`(path/int/float/str/form/delta) + `SyncPerksSpec`(storage/key/nodes[]) + `SyncPerkNodeSpec`(path/perk/minRank)；掛在 `DialogueSpec.Persist` / `.SyncPerks` |
| Gen | `Generator.JContainers.cs` | **JFormDB 生成**（Idea #20 持久層，design §七 Phase 0 / U5）：`JFormDbKeyExpr`(speaker→akSpeakerRef、player→Game.GetPlayer())、`JFormDbPath`(`.<storage><sub>`)、`JContainersPropertyDecls`(Form/Perk Auto 宣告)、`JContainersFragmentBody`(persist→`JFormDB.solveXxxSetter`〔delta 走 read-add-write〕；syncPerks→`solveInt>=minRank ? AddPerk : RemovePerk`)、`HasPersist`/`HasSyncPerks`、`PersistFormProperty(i)`=`PF_<i>`/`SyncPerkProperty(i)`=`SyncPerk_<i>`。**只用 root-DB path API → 無 retain/release（U5 設計上繞開）**；純字串、離線可測 |
...
| Validate | `Generator.Validate.Quests.cs` | dialogue 規則；**`hello:true` 招呼免 prompt**（招呼非玩家選項；其餘 dialogue 仍須 prompt）；**`setPrimaryIdentity` 須是已知 id 或 `auto`**；**`rewardItem` CheckRef**；**`persist`/`syncPerks`：storage 非空、key∈{speaker,player}(`IsPersistKey`)、persist entry 恰一值型(int/float/str/form)、delta 限 int/float、form/perk CheckRef**|
