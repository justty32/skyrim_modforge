...
| `examples/storage_json_ref_diag_spec.json` | **J組 storageWrites 兩擴充 diag（2026-06-22，.pex 已主力機編+結構驗）**：StartGameEnabled startUpStage-10 同時試 **arbitrary-ref target**（`target: MFSW_TestNpc`→`SWRef_0` Form prop，per-NPC）+ **`fromJson`**（int/str 從 `ModForgeJsonDiag/config.json` 經 JsonUtil 讀，literal 作 missing default）；`setstage MFSWJR_Q 10` 重觸發。交付 zip 的 `.pex` 含手注入的 Debug.Notification readback（npc-ref/player-ref/jsonDiff/jsonName）；隨包 `config.json`（difficulty=42,name=from-json-ok）→ 改/刪鍵驗 fallback。**runtime 待主力機進遊戲看通知**（見 WAIT_USER）|
