...
| `examples/scene-setstage.json` | 受限 scene phase fragment：phase OnStart 對綁定 quest 呼叫 `SetStage(stage)`；省略 quest 時用 scene host，不接受任意 Papyrus body |
...
| `examples/storage_json_ref_diag_spec.json` | **J組 storageWrites 兩擴充 IN-GAME CONFIRMED（2026-06-22）**：StartGameEnabled startUpStage-10 同時試 **arbitrary-ref target**（`target: MFSW_TestNpc`→`SWRef_0` Form prop，per-NPC）+ **`fromJson`**（int/str 從 `ModForgeJsonDiag/config.json` 經 JsonUtil **Path API** 讀，literal 作 missing default）。實機 `ref(npc)=1 / ref(player)=-999 / jsonDiff=42 / jsonName=from-json-ok`。**fromJson 真因坑**：`GetIntValue` 對外部手寫 JSON 是空的→必用 `GetPath*Value(".key")`（診斷探所有組合定死）。交付 zip 的 `.pex` 含手注入 5s-loop Debug.Notification readback；隨包 `config.json`（difficulty=42,name=from-json-ok）|
...
| `ConditionPlacedRefTests.cs` | **condition 的 `param`/`reference` 指 placed ref**（檔內 `placements[]` editorId ／ `references[]` label）在四個 deferred 呼叫點都解得到：perk（trunk＋effect）／storyEvent（＋locationFilter 仍排在後面的順序釘）／quest alias `findMatching` match filter（SM 與 standalone 兩條路）／scene＋phase；外加負向釘（解不到仍警告、perk effect 條件全滅不掛空 `PerkCondition` tab、`IsSceneActionComplete` 的 owningScene 預設仍在）|
...
| `SceneTests.cs` | SCEN actor / phase / dialogue action；非對話 action（Package→Packages PACK ref / Timer→TimerSeconds）+ beat phase（無 lines→無 Dialog action/topic）+ LastActionIndex；**fragment-backed action 發 Timer（idle hold / SetStage phase runner；純 build 無 VMAD）**；autoStart → controller VMAD 掛接 + 清 BeginOnQuestStart + 調參 props + **重播策略 props（playOnce/playHour/gateGlobal→GLOB object prop）** + validate gate |
...
| Build P1 | `Generator.Build.Dialogue.cs` | dialogue Branch + Topic + INFO（greeting 在 `Generator.Build.Dialogue.Hello.cs`）|
...
| Build P1 | `Generator.Build.Dialogue.Hello.cs` | **招呼專屬檔**（2026-08-27 由 `Generator.Build.Dialogue.cs` 拆出）。`BuildConditionedHello` 只建 INFO 並暫存於 (speaker\|quest)，`BuildNpcHellos` 才真正發 Hello topic（含 `MakeHello` local ＋ 只有 `greeting` 沒有 `dialogue[]` 的 NPC 的自建 host quest）。**`hello:true` 招呼：同一 (NPC,quest) 的所有招呼合進 **一個** Hello topic 的多條 INFO(conditioned 在前、plain `greeting` 墊底)——**鐵律:招呼靠 INFO 順序選取(第一個條件符合的勝出),不是多個獨立 Hello topic 靠 priority 競爭(那樣引擎只取一個 topic、其他無視);vanilla 237/297 Hello topic 多 INFO**(in-game 2026-06-07 確認身份招呼)|
...
| Build P1 | `Generator.Build.Dialogue.cs` `BuildDialogue` | **拆檔後只是編排器**（2026-08-27）：逐筆分派到 `BuildConditionedHello`／`BuildPlayerTopic`，再依序 `BuildDialogueViews`（DLVW）、`BuildNpcHellos`。**呼叫順序即 FormID 順序，不可重排**（golden-hash 197/197 把關）|
...
| 層次 | 檔案 | 職責 |
|-----|-----|-----|
| Spec | `Spec.Scene.cs` | `SceneSpec`, `SceneActorSpec`, `SceneAutoStartSpec`（在場偵測調參 + **重播策略 playOnce/playHour/playHourTolerance/gateGlobal**）, **`SceneActionSpec`（§1b 非對話 beat：`idle` 播動畫 / `setStage` 推任務 / package 走位 / timer 停頓 + phase 窗）**、`SceneSetStageSpec`（quest 可省略為 host + ushort stage）, **`ScenePhaseSpec`（含 `HeadtrackActor`/`HeadtrackPlayer`/`FaceTarget` per-phase gaze 控制 + `StartConditions`/`CompletionConditions` per-phase CTDA 閘）**, **`SceneSpec.Conditions`（scene-level CTDA：整個 scene 通過才啟動）**（從 `Spec.Dialogue.cs` 拆出；共用其 `ConditionSpec`）|
...
| Gen（scene phase actions）| `Generator.SceneFragments.cs` | scene phase-fragment **純產生器**（第三種 fragment 家族）：PlayIdle + restricted SetStage；按 phase 合併為單一 `Fragment_<phase>()`，分別呼叫 `GetActorRef().PlayIdle()` / 綁定 `Quest.SetStage()` |
| Build P2 | `Generator.Build.SceneFragments.cs` `AttachSceneFragments` | 僅 `SF_<scene>.pex` 在 `CompiledScriptsDir` 才掛 SCEN `SceneAdapter` VMAD；每 fragment phase 一個 OnStart fragment，綁 `Idle_<p>`/`Actor_<p>` 與 `SetStageQuest_<actionIndex>`（explicit ref 或 host quest）|
| Wire P2 | `Generator.Build.Scene.cs` `WireScenes` | actor alias→UniqueActor；**Package action 的 PACK ref → `action.Packages`（sceneActionWires）；controller GateGlobal → GLOB object prop（sceneGateWires）；scene-level + per-phase conditions（sceneConditionWires，phaseMap 對齊 spec-phase→built ScenePhase）→ **不在此建，改 `DeferCondition` 排隊**（WireScenes 跑在 placements 之前；scene gate 合法指 placed ref，如「玩家靠近那張椅子才開演」），aliasIdx＋owningScene FormKey 隨隊帶走，由 `WireDeferredConditions` 補建** |
...
| Gen | `Generator.StorageWrites.cs` | **J組 PapyrusUtil StorageUtil per-Form KV 生成**（**host-agnostic** — 對話 TIF 與 stage fragment 共用）：`ClassifyStorageTarget`(speaker/player/none〔none/global 皆→None〕/**Ref**〔其他 token＝任意 ref〕)、`StorageTargetExpr(target, refProp)`(speaker→akSpeakerRef、player→Game.GetPlayer()、none→None〔純表達式〕、**Ref→bound Form property**)、**`StorageRefProperty(prefix,i)`=`<prefix>SWRef_<i>`**、**`StorageRefEntries(writes)`**(ref-target 的 (index,spec)，binding 用)、**`StorageWritesPropertyDecls(prefix,writes)`**(每 ref entry 出 `Form Property` 宣告)、`StorageWritesBody(writes, prefix="")`(int/float→`Set{Int,Float}Value`〔delta→`Adjust…Value`〕；str→`SetStringValue`〔無 delta〕；**value 經 `StorageJsonOrLiteral` — `fromJson` 時→`JsonUtil.GetPath{Int,Float,String}Value(file, `JsonPath(key)`〔裸 key 補前導 `.`〕, <literal 作 missing default>)`；用 Path API 非 `Get…Value`（後者只讀 JsonUtil 自寫扁平 ns、對外部手寫檔是空的——2026-06-22 實機確認）**)、`HasStorageWrites`(List/DialogueSpec/StageSpec overloads)。共用 `EscapeStr`/`PapyrusFloat`。**speaker/player/none body-only 不綁 property；arbitrary-ref target 綁一個 Form property/entry（仿 persist key）**。**編譯需 PapyrusUtil .psc 上 header path（含 `fromJson` 時的 `JsonUtil.psc`）**；純字串、離線可測 |
| Gen | `Generator.QuestFragments.cs` `GenerateQuestFragmentSource` | stage fragment 純產生器；**`QuestNeedsFragmentScript` 含 stage persist/sync/storageWrites + `StoryTrigger`**；`StagePropPrefix(idx)`=`S<idx:D4>_`；每有 persist/sync/**storageWrites** 的 stage 進 `stageNums`，prop 宣告（含 **storageWrites ref 的 `StorageWritesPropertyDecls(StagePropPrefix(idx), …)`**）+ `Fragment_Stage_XXXX` body 末尾接 `JContainersFragmentBody(...)` + **`StorageWritesBody(st.StorageWrites, StagePropPrefix(idx))`（J組）**；**SM quest 兩者皆路由到 `OnStory<Event>` handler（`AppendJcStages`/`AppendStorageStages`），stage fragment 不發**（player/none/ref target，無 akSpeakerRef）|
...
| 層次 | 檔案 | 職責 |
|-----|-----|-----|
| Build P2 | `Generator.Build.Conditions.cs` `BuildCondition` ＋ `Generator.Build.Conditions.Functions.cs` `BuildConditionData` | 所有 CTDA 的 ref／比較／run-on 組裝 + function dispatch（dialogue / stage / banter / package / perk / SM / alias / scene / objective / recipe 共用）；163 行 dispatch 依 partial 命名拆到 `.Functions.cs`。🔑 **鐵律（檔頭寫死）**：CTDA 的 `param`／`reference` 是**任意 ref**，可以合法指一個 **placed ref**（檔內 `placements[]` editorId 或 `references[]` label）；那些 editorId 到 `BuildPlacements`/`BuildReferences`（Build.cs pass-2 ~115-117）才進 ref 表 ⇒ **任何跑在 `BuildReferences` 之前的步驟一律不准直接呼叫 `BuildCondition`**，必須用 `DeferCondition()` 排隊。`refsIndexed` 旗標（`BuildReferences` 開頭設起）是**守門員**：早於它呼叫就吐 `BUILD-ORDER BUG` 警告——不要消音，去 defer。**地點感知（#5）：`GetKeywordDataForCurrentLocation`（玩家當前地點 LocType）/`LocationHasKeyword`/`LocAliasHasKeyword`（location alias 的 keyword，hold 偵測，需 owning quest alias index，仿 GetIsAliasRef）。****`GetIsAliasRef`**：用 `alias`（owning quest 的 alias 名）→ alias index，由各 quest-scoped 呼叫點傳入 `aliasIndexByName`。**`IsSceneActionComplete`**：`BuildCondition` 第 4 參 `owningScene` FormKey，scene-cond 呼叫點（`WireScenes` 的 sceneConditionWires）傳 `scene.FormKey`，author 可用 `c.Scene` 覆寫；`c.SceneActionIndex` 必填。package/perk 等無 scene/quest context → 傳 null → 警告丟棄。**`GetVMQuestVariable`/`GetVMScriptVariable`（L組）**：condition 讀 Papyrus property，`param`=quest（VM-quest）/object（VM-script）、`c.VariableName`=property 名字串（verbatim 寫進 CTDA，引擎期望的 bare-name vs `::Prop_var` 待 xEdit/實機驗）|
| Build P2 | `Generator.Build.Conditions.Wire.cs` `DeferCondition` / `DeferConditionFinalizer` / `WireDeferredConditions` | **deferred-condition 佇列**（上面那條鐵律的落實）。早於 placements 的四個呼叫點（`WirePerks` / `BuildStoryManager` / `BuildQuestAliases`＝SM＋standalone 共用的 `WireAliasMatchConditions` / `WireScenes`）把 `ConditionSpec` 連同 target `IList<Condition>`＋label＋aliasIdx＋owningScene 排進 `deferredConditionWires`；`WireDeferredConditions`（Build.cs 排在 `WirePackageConditions` 之後、`WireDeferredScriptObjectProps` 之前，即 placements＋labels 都在了）依**排隊順序**排空 ⇒ 每個 target list 的 CTDA 順序與舊的 eager 版**逐位元相同**。`DeferConditionFinalizer` 給「條件全建不出來就不該掛的容器」用（perk effect 的 `PerkCondition` tab——vanilla 不發空 PRKC）|
