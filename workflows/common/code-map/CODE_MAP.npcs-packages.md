...
| 層次 | 檔案 | 職責 |
|-----|-----|-----|
| Spec | `Spec.CapturedNpcs.cs` | `CapturedNpcSpec`（逐欄對齊 DLL json；含 class/level/combatStyle/voiceType/spells/inventory）＋`CapturedNpcItemSpec`（{item,count,worn,name,enchantment?}——**實例附魔列**）＋`CapturedHairColorSpec`（只消費 id）＋`CapturedNpcPerkSpec`（rank advisory）＋`CapturedActiveEffectSpec`（整組 advisory）|
