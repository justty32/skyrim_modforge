...
- **exterior 商店 vendor：VendorLocation 用 ref-anchored、chest 要放 leveled 貨**（2026-07-08）：自訂 NPC 商人 barter 開了卻空（連 chest 的金都不顯示）＝ `GetOffersServicesNow` 恆 0 → merchant container 沒被讀。兩坑：① **VendorLocation**——`WireDeferredMerchantContainers` 原只從 `placement.Cell` 設 `LocationCell`,但 **exterior 擺放用 `Worldspace`、無 named cell**(且 build 會回填 cell 成讀不回的 link)→ VendorLocation 空。修:worldspace 擺放改用 **ref-anchored `LocationTarget`(錨 chest REFR)+ 實 radius**(~4096;radius 0 是退化點,永遠沒人「在」內)= 店周圍售貨球。interior 仍走 LocationCell。② **庫存**——chest 只放裸金 → 沒東西買;要放 vanilla 商人 **leveled lists**(`VendorGoldBlacksmith` 0x072AE9 給金 + `LItemBlacksmith{Weapon,Armor,SpecialLoot}100`)才有貨。③ **營業時間**——vendor hours(如 8-20)也進 GetOffersServicesNow,夜間測不開,需白天。
