...
← [investigation README](README.md)｜工作區 [`sub_projs/mod-survey/`](../../sub_projs/mod-survey/README.md)

給負責「調查 `~/skyrim_mods/` 那批已下載 mod」的 AI agent。**產出寫進 `sub_projs/mod-survey/`**，不要寫進 `workflows/`（主 session 正在重構它）。
...
- 預抽好的文本：`sub_projs/game-data/mods/<plugin>/`（先查這裡，多數內容型 mod 的對白/書/任務已在）
...
3. **抽文本**：`sub_projs/game-data/extract.sh <path/to.esp>` → `game-data/mods/<name>/`，讀 `summary.txt` 抓規模，再讀 `dialogue.md`/`quests.md`。
...
很多隨從/對話 mod（RDO、FCO、IFDL、Nether's Follower Framework…）的 pattern 直接關係到 Sofia patch 的相容性與借鏡。發現可用 pattern 時，在 finding 裡明確指向 `sub_projs/sofia-patch/` 的對應功能（F1–F16，見其 `expansion-plan`）。
