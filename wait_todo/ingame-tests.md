...
- **【Idea #20 Phase 1】in-world 技能樹 — ✅ 零外部依賴 standalone 版（2026-06-21，使用者明確不想裝 Campfire/Frostfall）**：交付 `~/skyrim_mods/mine/ModForgeSkillTree.zip`（FLAT：esp 在根 + `Scripts/MFSkillNode.pex`+source）。spec＝`examples/inworld_skill_tree_standalone_spec.json`，腳本＝`examples/MFSkillNode.psc`（已用 native compiler 對 vanilla header cache 編綠）。**只依賴 Skyrim.esm**。
  - 不是 Campfire 那種營火 radial menu，而是**最直白的 in-world 樹**：一個自訂房間裡擺 3 顆漂浮水晶節點（vanilla `WispCrystal01.nif`），各掛我自己寫的 `MFSkillNode` 腳本（純 vanilla 型別）。點擊 → gate 檢查（前置節點 + 點數）→ 給一個自訂 Fortify ability + 扣 1 點。垂直鏈：Forged Resolve（root，永遠可點）→ Forged Vigor → Forged Mastery（各給 Fortify Health/Stamina/CarryWeight）。MFSkill_PointsAvail 預設 3。
  - **結構已全驗**（dump 輸出 esp）：**masters 只有 Skyrim.esm（零 Campfire/Frostfall）**；3 個 ACTI 的 MFSkillNode 屬性全解析無 null；gating 鏈正確（Node0 無 prereq、Node1.prereq=N0_Rank、Node2.prereq=N1_Rank）；3 個 Ability/ConstantEffect/Self spell 包對應 ValueModifier MGEF；cell=interior + RiftenInteriors LGTM；floor（`WRIntFloorSTMid01Large` 0x1044AA）+ 3 節點就位。
  - **怎麼測**：裝 zip+啟用 → 進遊戲 → console `coc MFSkillTreeRoom` → **環顧四周找 3 顆漂浮發光水晶**（一條垂直線，最低那顆在正前方）→ 準心對**最低**那顆按 [E] → 跳通知「Learned: Forged Resolve — 2 point(s) left」+ Magic>Active Effects 出現 Fortify Health → 再點中間那顆（應可，因 root 已學）→ 最上面那顆。**先別點 root 直接點中間/最上 → 應跳「Locked. Learn the node below it first.」**（驗 gating）。
  - **三種結果**：① **看得到水晶 + 可點 + 學到 ability（Active Effects 出現）+ gating 正確（跳過前置會擋）** → Phase 1 過，「JSON spec → 可點 in-world 節點樹 + 自編腳本 + 給效果」整條成立，放心寫 Phase 3 generator。② **掉出地板/沒站穩** → 落地後 `tcl` 關碰撞飛到水晶前再點（或回報我把 floor 調大/加 coc marker）。③ **水晶看不到（隱形）/ 點了沒反應 / CTD** → 回報我 + `Papyrus.0.log`（隱形＝mesh 路徑問題、沒反應＝腳本沒綁上或 .pex 沒進去）。
  - **快速隔離**：console `getglobalvalue MFSkill_N0_Rank`（點前 0、點後 1）、`player.getav Health`（學 Resolve 後 +50）。
  - **備註**：依賴 Campfire 的另一版（`examples/inworld_skill_tree_spec.json`，營火 radial menu）仍留在 repo 當 Phase 3 generator 的設計範本，但**不交付、不需安裝**。
