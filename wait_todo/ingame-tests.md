...
- **遊戲內場景匯出 · blacksmith role macro（Idea #24 §D，2026-07-08）** — ModForge 側 M0–M1 已完成、離線全綠（856 tests）、端到端 build 已驗。**純記錄 patch，無 .pex／無 fragment**（Hello 對話 + sandbox package + NpcPatch 全無腳本）→ 打包只要 build + FLAT zip，不必 Papyrus toolchain。
  - **打包**：`dotnet run --project src/ModForge.Cli -- build examples/scene-export-blacksmith.scene.json <stage>/ModForgeSceneBlacksmith.esp` → 連同 `Seq/` FLAT zip → `~/skyrim_mods/mine/`。（例子把 Carlotta 0x013B99 當 clone 替身，故 master=Skyrim.esm。）
  - **最簡實機驗（新遊戲或 save+reload 吃 .seq）**：進白漫城市集找 **Carlotta Valentia** 對話 → 應聽到問候 **「Need something forged? You've come to the right anvil.」**（我們的 Hello 是 GetIsID→Carlotta gated；語音空＝字幕短暫顯示 + mumble，正常）。這驗證「貼 role → ModForge 生該 NPC 的 conditioned 問候」的核心。
  - **次要**：例子另在 Tamriel (0,−9000,−3800) 附近擺了 Farmhouse01 + 一個 Carlotta 複本 + town 地圖 marker「Forgewatch」（可快旅）+ campfire hazard 特效——`coc` 或快旅過去看房子在不在、marker 能不能快旅、篝火特效可見否、複本 Carlotta 也講鐵匠問候。
  - **回報**：Carlotta 有沒有講鐵匠問候（核心）；房子/marker/特效在不在（§B/§C）。**這是 idea #24 ModForge 側管線的第一個實機證據**。哪項不動我來定位。
  - **仍缺（非本切片，已知）**：vendor 服務（NpcPatch 不能加 faction，見 spec §D，待補 faction-add）；範圍吸取／移除物件的生成（`removals[]` 小 GAP，隨 M4 採集橋補）；controller 的縮放/位移 mode（runtime，合流 settlements P2）。
