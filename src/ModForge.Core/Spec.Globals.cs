...
// readable by CONDITIONS with zero Papyrus (GetGlobalValue) as well as by Papyrus (GetValue/SetValue)
// and the console (set/show). The plugin's `value` is the INITIAL value only — an existing save keeps
...
// COUNTER, a chance/weight (region/leveled-list), or a tuning CONSTANT (mark `constant` for read-only).
