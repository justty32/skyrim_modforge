internal static partial class Program
{
    ...
    private static int QuestDiag(string inPath, string formIdHex)
    {
        ...
        {
            if (q.FormKey.ID != id) continue;
            Console.WriteLine($"0x{id:X6}  EditorID={q.EditorID}  flags={q.Flags}  priority={q.Priority}");
        ...
        Console.WriteLine($"0x{id:X6} not a Quest in {Path.GetFileName(inPath)}");
        return 0;
    }
}
