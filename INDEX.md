...
| `scripts/` | 工作流 bash：出貨與環境（bootstrap-pex / ship / ship-voice / extract-skyrim-masters）＋**測試四支**（test-offline 跑測試；golden-hash / cli-dispatch-snapshot 兩支重構護欄；coverage 找沒測到的洞）——後者用法見 [testing.md](workflows/testing.md) |
