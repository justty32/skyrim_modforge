...
| Custom dialogue records are valid but the topic NEVER appears when you talk to the NPC | Two flags missing: the host **Quest isn't Start Game Enabled** (so it never runs → its dialogue is never loaded) and/or the **DialogBranch isn't Top-Level** (so the topic is a sub-branch, not a menu option) | Quest `flags |= Quest.Flag.StartGameEnabled` (+ a `Priority`); branch `Flags = DialogBranch.Flag.TopLevel`. In ModForge these are `quests[].startGameEnabled` (default true) and automatic on the dialogue branch (It.23) |
| Custom dialogue line shows no audio / NPC lips don't move | No recorded voice (.fuz/.lip) — expected for generated dialogue | Turn **General Subtitles + Dialogue Subtitles ON** (Settings ▸ Display) — the line still shows as a subtitle and the menu option still works; that's a valid in-game confirmation |
| Want a hireable follower but the "Follow me" topic never appears | NPC isn't in **PotentialFollowerFaction**, and/or has no **Ally relationship** to the player — the vanilla free-follow dialogue is gated on both | Add `Skyrim.esm:0x05C84D` to `factions` + a `relationships[]` entry (parent=NPC, child=`Skyrim.esm:0x000007`, rank=`Ally`) + a follower-capable voice (MaleNord/FemaleNord). Leverages the vanilla DialogueFollower quest — no custom dialogue/scripts (It.24) |
