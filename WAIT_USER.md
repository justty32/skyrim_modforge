...
- **[ingame-tests.md](wait_todo/ingame-tests.md)** — 純遊戲實機測試（含**怎麼測通用流程** + MO2 鐵律）。**目前最優先**：scene-capture-bridge 的 🐞 **`isPlayer` ＋ 玩家 perk 修正**（DLL `dd7afd82` 已部署，對帳錨點 `Skyrim.esm:0x0F2CAA`）；接著 **`ModForgeGoblets.zip`**（2026-07-13 交付，`noHavokSettle` 的最終驗收：學院庭院 8 顆銀杯，自帶對照組）。其餘：`sc capp` 數值（要**練過的角色**）、模式開關 `py`/`ed`/`pkc`、referrer 剩下的三項、動作鍵 `.ini` ＋ palette clear、darksouls-port P1、living-adventurers、blacksmith 場景、VNML、Sofia × VIGILANT Act 1 / Act 2-4。
