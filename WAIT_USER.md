...
- **外部資源**（Nexus 下載 mod / 外部工具實跑）：
  ...
  - **【動態生怪 F組 #3】MFDynamicSpawn navmesh 吸附 runtime 驗證 — 待主力機**：✅ 已離線實作 `quest.spawn`（`{form,count,minDistance,maxDistance,snapToNavmesh}`）→ 掛 reusable `MFDynamicSpawn.psc`（extends Quest，OnInit `PlaceAtMe`+`MoveTo` 玩家附近隨機偏移 + `EnableAI` toggle 吸 navmesh，EE NavmeshTester trick）。record/VMAD/property 接線離線測過（8 測綠），與 #5/#6 共存 example `examples/location_encounter_spec.json`。⚠️ **待你做（主力機）**：① 編 .pex（`dotnet run -- compile assets/papyrus/MFDynamicSpawn.psc assets/papyrus/`，同其他 MF* controller）；② **runtime 驗**——build 一個帶 `spawn` 的 quest，進遊戲觸發，確認怪物生在玩家附近合法可走點（`PlaceAtMe`+`MoveTo` 偏移、`EnableAI` toggle 吸 navmesh、`Math.Sin/Cos` 角度這些離線無法跑驗；尤其 EnableAI 吸附是否真把懸空/卡牆的 actor 拉到地面）；③ example 的 EncBandit01 FormID 是真 vanilla，但用 `gamedata find` 複核。收尾驗證，不擋使用。
