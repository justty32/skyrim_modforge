...
public class McmGenTests
{
    ...
    {
        var files = McmGen.Generate(Sample());
    ...
    {
        using var doc = JsonDocument.Parse(McmGen.BuildConfigJson(Sample()));
    ...
    public void GroupConditionNot_EmitsNotObject()
    {
        ...
        using var doc = JsonDocument.Parse(McmGen.BuildConfigJson(m));
