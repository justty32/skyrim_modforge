...
public class RequiresTests
{
    ...
    // The SHIPPED-mod form (what `package` drops into the mod folder) is read by a PLAYER, not the author:
    // it must still name every mod to install first — with each one's reason / version / link — but drop
    // the spec-field attribution and "remove the line and rebuild" advice, which mean nothing without the
    // spec. The author-facing form (what `build` writes) keeps them.
    [Fact]
    public void ShippedForm_KeepsTheInstallList_ButDropsSpecInternals()
    {
        var papyrusUtil = new RequirementSpec { Name = "PapyrusUtil SE", Reason = "storageWrites", Url = "https://nexus/13048" };
        var spec = CaptureSpec(
            new RequirementSpec { Plugin = "PROTEUS.esp", Version = ">=3.4", Url = "https://nexus/proteus" },
            Req("XPMSE.esp"),
            papyrusUtil);
        var result = TestBuild.Ok(spec);

        var author  = Generator.RequiresFileText("MFCap.esp", result.Dependencies, spec.Requires)!;
        var shipped = Generator.RequiresFileText("MFCap.esp", result.Dependencies, spec.Requires, forShippedMod: true)!;

        // The player still learns exactly what to install, and where + which version.
        Assert.Contains("PROTEUS.esp", shipped);
        Assert.Contains("PapyrusUtil SE", shipped);           // the SKSE framework with no plugin of its own
        Assert.Contains("https://nexus/proteus", shipped);    // declared url survives
        Assert.Contains(">=3.4", shipped);                    // declared version survives

        // Player-facing header, not the author "generated by build".
        Assert.Contains("INSTALL REQUIREMENTS", shipped);
        Assert.DoesNotContain("generated by `modforge build`", shipped);

        // The spec internals the author form carries are absent from the shipped form.
        Assert.Contains("capturedNpcs[0].spells", author);        // author form HAS the field attribution…
        Assert.DoesNotContain("capturedNpcs[0].spells", shipped); // …the shipped form does NOT
        Assert.DoesNotContain("rebuild", shipped);                // no "delete the spec line and rebuild"
    }
